...
export async function POST(request: NextRequest) {
  ...
  let body: SerenaPostUpsertInput

  try {
    body = (await request.json()) as SerenaPostUpsertInput
  } catch {
  ...
  payload.logger.info(
    `[serena] post ${existing ? 'updated' : 'created'} slug=${result.slug} status=${result._status} ip=${auth.clientIp || 'unknown'}`,
  )
